...
Deliverable lifecycle is also an explicit runtime adaptation: the canonical Deliverable schema does not define the runtime's `active|retired` projection. Human reviewer/dispute handling, governed recurring-service closure, live occurrence reconciliation, bounded receipt archival, lossless KPI history, and financial EVM/EQV metrics remain incomplete. Task QI/DQI/P3-PQI formulas now follow the documented fail-closed behavior in `docs/adr-fspm-eqvm-quality-rollup.md`, while the concrete recurring Activity-weight policy remains unapproved and canonical scores remain null. Remaining limitations stay visible in the machine-readable conformance profile and issues #136, #164, and #194.
