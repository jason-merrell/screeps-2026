...
The production runtime build is intentionally excluded from Turborepo's artifact cache. Its embedded provenance is derived from the candidate Git state, which is outside a normal source-content cache key; replaying a bundle built before the candidate commit could otherwise publish a stale or null runtime SHA.
