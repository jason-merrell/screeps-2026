...
Planner creation performs a global identity-registry preflight before any ensure/defaulting mutation. That traversal builds a tick-local index and live-reference witnesses, then installs non-serialized mutation-aware guards over every authority identity, canonical registry slot, and Procedure collection. Later trace requests validate their requested Empire-to-Procedure spine in constant indexed lookups and materialize only missing records on that spine. A raw same-tick change anywhere in the indexed hierarchy invalidates the transaction; a global rebuild exposes malformed or ambiguous identity before the requested trace can write, while a semantically canonical replacement is still rejected as out-of-band mutation. Canonical registry topology is closed for the rest of the tick, then restored to equivalent extensible plain objects once at tick rollover before perception can discover colonies. Newly materialized records guard only their constant-size requested spine, never rescan the global hierarchy.

Colony storage keys, P3, Requirement, Deliverable, Task, and Procedure identities and placements must be canonical and globally unambiguous. A malformed existing Empire or colony authority container is preserved for diagnosis, but cannot create traces or authorize execution; authority is never fabricated as a repair. Only versioned migration may add the P3 to a legacy pre-P3 colony portfolio. Tick snapshots reject cross-tick reuse and same-tick hierarchy removal or replacement rather than authorizing through detached references.
...
Compatibility initialization may populate a genuinely absent non-root field, such as a legacy Requirement with no `p3Id`. It must preserve and reject an existing contradictory `p3Id` or colony `parentP3Id`. An entirely absent current colony portfolio may be initialized, but an existing portfolio missing its P3 is corruption and must remain quarantined. Planner trace creation first performs a read-only guard: unknown catalog Task/Procedure keys, incomplete Active catalog Procedure sets, inactive existing ancestors, and contradictory Portfolio, Requirement, Deliverable, Task, or Procedure identities throw before any defaulting code can touch Memory. A Retired Task record, including its definition fields and Procedure history, is never canonicalized or appended. Legacy Service Program and synthetic contract records remain readable historical evidence but never substitute for the active P3 chain.
