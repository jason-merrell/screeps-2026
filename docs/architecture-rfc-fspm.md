...
The execution kernel now has durable, content-pinned Corporate Requirement and Corporate Deliverable authority plus Task, Procedure, Activity, Intent, and outcome evidence. The source-controlled authority package is an explicit Screeps service-principal adaptation; it is not canonical human approval. The versioned conformance profile in `docs/fspm-conformance.json` remains authoritative about incomplete OU/ARCI authority, general child-Deliverable decomposition, the unapproved recurring-Activity weighting/reporting policy, human-reviewed/lossless EQVM history and financial EVM metrics, continuously managed Portfolio decisions, and issue/risk/stakeholder governance.
...
The quality roll-up formulas and evidence checks are fail-closed: terminal Activity samples cross-verify their Activity, Task, actor, type, completion tick, rating, and evidence. Every canonical Task QI, DQI, and PQI binds to discriminated policy authorization, but an approval-shaped nested record is not authority. Numeric complete quality requires resolution from a governed approval ledger that binds the exact event, policy, authority, signer, effective tick, and complete rated Activity cohort. No such EQVM ledger exists in the current package, so runtime trace and snapshot boundaries reject every approval-shaped numeric claim. The concrete equal-weight/latest-24 policy is also an unapproved research configuration outside governed Task records. Canonical Task QI is therefore unavailable, and DQI/PQI remain null; any numeric diagnostic is separately named research telemetry. The authorization debt and remaining adaptations are recorded in `docs/adr-fspm-eqvm-quality-rollup.md`.
