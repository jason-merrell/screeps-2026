...
export default async function OperatorPage({ searchParams }: OperatorPageProps) {
  ...
  return (
    ...
    >
                  ...
                  <div className="rounded-xl border border-primary/20 bg-primary/7 p-4 text-sm">
                    ...
                    <div className="mt-3 text-xs leading-5 text-muted-foreground">
                      {params.wake === "dispatched"
                        ? "Trusted worker dispatched immediately. Lifecycle status will update below."
      ...
      {isOperator ? (
        <Card className="lab-panel mt-5 rounded-2xl border-white/8 bg-card/65">
          <CardHeader className="border-b border-white/8 pb-5">
            <div className="flex flex-col justify-between gap-4 lg:flex-row lg:items-end">
              <div>
                <CardDescription className="text-[0.68rem] uppercase tracking-[0.18em]">Command ledger</CardDescription>
                <CardTitle className="mt-1 text-xl">Recent activity</CardTitle>
                <CardDescription className="mt-1">Your latest commands and their authoritative lifecycle events.</CardDescription>
              </div>
              <div className="grid grid-cols-3 gap-2 text-center text-xs">
                <div className="min-w-24 rounded-xl border border-white/8 bg-black/10 px-3 py-2"><div className="text-muted-foreground">Active</div><div className="mt-1 text-base font-semibold">{activeCommands.length}</div></div>
                <div className="min-w-24 rounded-xl border border-white/8 bg-black/10 px-3 py-2"><div className="text-muted-foreground">Success</div><div className="mt-1 text-base font-semibold">{terminalCommands.length ? `${Math.round((succeededCommands.length / terminalCommands.length) * 100)}%` : "—"}</div></div>
                <div className="min-w-24 rounded-xl border border-white/8 bg-black/10 px-3 py-2"><div className="text-muted-foreground">Avg claim</div><div className="mt-1 text-base font-semibold">{formatDuration(averageClaimLatency)}</div></div>
              </div>
            </div>
          </CardHeader>
            ...
            {recentCommands.length ? (
              <div className="divide-y divide-white/8">
                {recentCommands.map((command) => {
                  ...
                  return (
                    <div key={command.id} className="grid gap-4 p-5 lg:grid-cols-[minmax(0,1fr)_auto] lg:items-center">
                      <div className="min-w-0">
                        ...
                        <div className="mt-3 flex flex-wrap gap-x-4 gap-y-2 text-xs text-muted-foreground">
                          ...
                          <span>Claim {formatDuration(claimLatency)}</span>
                          <span>Total {formatDuration(totalDuration)}</span>
                          {runId ? <a className="text-primary underline-offset-4 hover:underline" href={`https://github.com/jason-merrell/screeps-2026/actions/runs/${runId}`} target="_blank" rel="noreferrer">Worker #{runId}</a> : null}
                        </div>
                      ...
                      <div className="max-w-full truncate font-mono text-[0.62rem] text-muted-foreground lg:max-w-56" title={command.command_key}>{command.command_key}</div>
                    </div>
                  );
                })}
              </div>
